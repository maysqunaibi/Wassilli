<?php
    ...
    $query = "SELECT employees.firstName, SUM(products.buyPrice) FROM employees,customers,orders,orderdetails,products WHERE employees.employeeNumber= customers.salesRepEmployeeNumber AND customers.customerNumber=orders.customerNumber AND orders.orderNumber =orderdetails.orderNumber AND orderdetails.productCode=products.productCode Group BY(employees.employeeNumber) LIMIT  $start_from, $limit";
    ...
    if(mysqli_num_rows($res) < 0 ){
        ...
        $output .="<tr>
        <td>".$row['firstName']."</td>
        <td>".$row['SUM(products.buyPrice)']."</td>
